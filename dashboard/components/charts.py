...
def _calc_zx_lines(
    ...
    close = df["close"].astype(float)
    zxdq = (
        close.ewm(span=zxdq_span, adjust=False)
        .mean()
        .ewm(span=zxdq_span, adjust=False)
        .mean()
    )
    zxdkx = (
...
def _calc_brick(
    ...
    for i in range(length):
        ...
        var2a[i] = (
            (v1 + shift2)
            if i == 0
            else (a1 * v1 + b1 * (var2a[i - 1] - shift2) + shift2)
        )
        ...
        fig.update_layout(
            **{
                xname: dict(
                    ...
                    linecolor="#d0d7de",
                    tickfont=dict(
                        family="DejaVu Sans, Arial, SimHei, sans-serif", color="#636c76"
                    ),
                )
            }
        )
